...
interface AddPlaylistModalProps {
  onSave: (playlist: Omit<Playlist, 'id' | 'created_at' | 'updated_at'>, imageFile?: File) => void;
  onClose: () => void;
  isLoading: boolean;
}

export default function AddPlaylistModal({ onSave, onClose, isLoading }: AddPlaylistModalProps) {
  const [formData, setFormData] = useState<Omit<Playlist, 'id' | 'created_at' | 'updated_at'>>({
    title: '',
    cover_image: '',
    songs: [],
    platform_links: {}
  ...
  const validateField = (fieldName: string, value: any): string => {
    switch (fieldName) {
      ...
      case 'coverImage':
        return !imageFile ? 'Cover photo is required' : '';
      case 'songs':
  ...
  const validateForm = (): boolean => {
    ...
    errors.coverImage = validateField('coverImage', imageFile);
  ...
  const handleBlur = (fieldName: string) => {
    ...
    const error = validateField(fieldName, fieldName === 'coverImage' ? imageFile : 
                                 fieldName === 'songs' ? formData.songs :
      ...
      <div className="admin-modal admin-modal-large" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="admin-modal-body">
            ...
            <div className="admin-form-group">
              <label>Cover Image: <span className="required-asterisk">*</span></label>
              {imagePreview && (
              ...
              <input
                ...
                disabled={isLoading}
              />
              {fieldErrors.coverImage && <span className="field-error">{fieldErrors.coverImage}</span>}
              <small style={{ display: 'block', color: '#888', marginTop: '5px' }}>
                Upload a cover image for the playlist (Required)
              </small>
            </div>
            ...
            <div className="admin-form-group">
              ...
              {formData.songs.length === 0 && (
                <p style={{ color: '#888', fontSize: '0.9rem', marginBottom: '10px' }}>Click "Add Song" to start adding songs</p>
              )}
            ...
            <div className="admin-form-group">
              ...
              <input
                ...
                placeholder="https://gaana.com/playlist/..."
                disabled={isLoading}
              />
